	...
	function ServiceVPLEXProvisioningCtrl($scope, $filter, $http, $localStorage, httpService, $stateParams, $interval, $rootScope, MyData) {
		...
		$scope.initData = function () {
			...
			httpService.get("/auto/service/block/provisioning/getinfo", "", config, function (response) {
				...
				$scope.HostDeployLabels = response.HostDeploy.label
				angular.forEach(response.ProtectLevel, function (data, index, array) {
					if (data.name == "DR_SameCity") {
						...
						$scope.showDrSameCityLabel = data.label;
						...
						//}		
						$scope.showDrDiffCityLabel = data.label;
						...
						$scope.showDrBackupLabel = data.label;
						...
						$scope.showAppVerificationSameCityLabel = data.label;
						...
						$scope.showAppVerificationDiffCityLabel = data.label;
						...
						$scope.showLocalCdpLabel = data.label;
		  ...
		  function errorCallback(response){
		   if (response == 'timeout') {
				...
				alert('访问网络错误！错误代码:' + response);
			}

	  })*/
			/**httpService.get("/auto/service/block/provisioning/getinfo2", "", config,
			   function(response){
				debugger;
			   $scope.ApplicationList = response.Application;
			   $scope.AppNames = response.StorageResourcePool;
			...
			//$http.get("/auto/service/block/provisioning/getinfo", config)
		...
		$scope.$on('ToBrotherController', function (event, msg) {
			...
			if (msg == 1) {
				httpService.post("/auto/service/block/provisioning/review", json, config, function (response) {
					checkWebsocket = true;
					//nextResult = response.AutoInfo.ActionParamaters;
					nextResult = response;
				...
				httpService.post("/auto/service/block/provisioning/execute", nextResult, config, function (response) {
					checkWebsocket = true;
